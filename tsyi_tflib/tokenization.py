...
def mecab_bert(_str):
    _str = ' ' + str(bytes(_str,'utf8', 'ignore'), 'utf8', 'ignore')
    r = mecab_ko_dic.parse( _str )
    # 행단위로 분리한다.
    r1 = r.split('\n')
    # 분리된 어휘토큰(짝수번째 값)을 선택하여 결과를 저장한다.
    ret = []
    for tok in r1:
        if tok.count('\t') > 0:
            s =  tok.split('\t')
            ret.append(s[0])
            #ret.append( [s[0], s[1].split(',')[0]] )
            '''
            pos = s[1].split(',')[0].split('+')[0]
            if pos in ("JKS", "JKC", "JKG", "JKO", "JKB", "JKV", "JKQ", "JX", "JC", "EF", "EC", "ETN", "ETM",
                       "EP", "XSN", "XSV", "XSA"):
              ret.append('##'+s[0])
            else:
              ret.append(s[0])
...
class FullTokenizer_mecab_spm(object):
  ...
  def tokenize(self, text):
    if self.sp_model:
      mecab_tokens = mecab_bert(text)
      split_tokens = encode_pieces(self.sp_model, " ".join(mecab_tokens), return_unicode=False)
    else:
      split_tokens = []
      basic_tokens = self.basic_tokenizer.tokenize(text)
      for token in mecab_bert(" ".join(basic_tokens)):
...
class FullTokenizer_mecab(object):
  """Runs end-to-end tokenziation."""
  ...
  def split_words(self, text): # 예측에서 출력을 해석 할 때 사용함.
    basic_tokens = self.basic_tokenizer.tokenize(text)
    word_list = mecab_bert(" ".join(basic_tokens)) # 이 작업을 해서 학습 입력과 동일한 mecab 분해 단위로 단어가 쪼개 져야 함.
    return word_list

  def tokenize(self, text):
    split_tokens = []
    basic_tokens = self.basic_tokenizer.tokenize(text) # 기호 숫자 등 기본 쪼갬.
    for token in mecab_bert(" ".join(basic_tokens)): # mecab을 써서 추가로  쪼갬.
      for sub_token in self.wordpiece_tokenizer.tokenize(token):
        split_tokens.append(sub_token)
